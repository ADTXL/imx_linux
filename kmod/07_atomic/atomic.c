...
{
    /* 通过原子变量的值判断led有没有被使用，如果被使用则返回-EBUSY  */
    if (!atomic_dec_and_test(&gpioled.lock)) {
        atomic_inc(&gpioled.lock); /*小于0的话加1，使之为0*/
        return -EBUSY;
    }
...
static ssize_t led_release(struct inode *inode, struct file *filp)
{
    ...
    atomic_inc(&dev->lock); /*关闭文件时加1，释放原子变量*/
    return 0;
}
